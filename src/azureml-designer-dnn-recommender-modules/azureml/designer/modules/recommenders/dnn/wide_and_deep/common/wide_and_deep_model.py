...
from azureml.designer.modules.recommenders.dnn.wide_and_deep.common.wide_and_deep_dataset import WideDeepDataset
from azureml.designer.modules.recommenders.dnn.wide_and_deep.train.train_log_hook import TrainLogHook
...
class WideAndDeepModel(metaclass=EntryParam):
    LEARNER_NAME = "Trained Wide and Deep recommendation model"
    ...
    def train(self, interactions: InteractionDataset, user_features: FeatureDataset = None,
        ...
        input_fn = training_data.get_input_handler(batch_size=self.batch_size, epochs=self.epochs, shuffle=True)
        log_hook = TrainLogHook(steps_per_iter=self.steps_per_iteration)
        module_logger.info(f"Start to train model.")
        try:
            model.train(input_fn=input_fn, hooks=[log_hook])
    ...
    def load(cls, path: str):
        try:
            with open(os.path.join(path, MODEL_NAME + '.pkl'), "rb") as f:
                model = pickle.load(f)
        except ModuleNotFoundError:
            raise InvalidLearnerError(arg_name=cls.LEARNER_NAME)
